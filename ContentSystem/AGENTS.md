...
- Header/footer resolution is isolated by the assignment's `DataScopeField`. The final null-domain/null-channel fallback means the default assignment inside the current platform or tenant scope; it never falls back from a tenant to the platform scope or another tenant.
