...
- `SKIP_VALIDATION_STATE` suppresses assignment validation on both sections when added to the write `Context` via `Context::addState`; intended for trusted bulk importers (no in-repo path sets it); the Frontend validator checks the flag identically to the Core `ContentLayoutAssignmentWriteValidator`
...
- Reverse dependency direction: no Core → Frontend dependency; the Frontend validator depends on Core's `LayoutRootSourceReader`, with no callback into Frontend
