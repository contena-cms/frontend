...
namespace Contena\Frontend\Resources\views\components\Button;
...
use Contena\Frontend\Resources\views\components\Button;
...
**Note:** Be careful with adding too much business logic inside component classes. At Contena we typically implement anonymous components that are pure frontend representations which get passed all necessary data via properties.


## Adding components via Contena extension

New components can easily be added by Contena apps and plugins.

### Anonymous Components
The anonymous components work right of the box and you don't have to do anything else than putting a template file under `<your-plugin>/src/Resources/views/components`. Both, apps and plugins can provide anonymous components in any distribution form of Contena. Your components will automatically be prefixed with the bundle name of your app or plugin.
...
* The root class uses the `ct-` prefix to be independent of existing/old components.
* Child elements must "follow" the root class naming.
* The BEM naming pattern is used.

✅ Do:
```html
<div class="ct-product-card card">
    <div class="ct-product-card__body card-body">
        <h2 class="ct-product-card__title">Card title</h2>
    </div>
    <div class="ct-product-card__footer card-footer">
    </div>
</div>
```
...
```html
<div class="ct-product-card card">
    <div class="ct-card-inner card-body">
        <h2>Card title</h2>
    </div>
    <div class="ct-card-bottom card-footer">
    </div>
</div>
```
...
The APIs of a component are "Props", "Blocks", "Attributes" and "Slots". Slots are an additional feature in Contena to allow component extension using the CMS.
...
<div class="ct-product-card card">
    ...
    <div class="ct-product-card__body card-body">
        {% block title %}
            <h2 class="ct-product-card__title card-title">{{ product.name }}</h5>
        {% endblock %}

        {% block price %}
            <p class="ct-product-card__price card-text">{{ product.price|currency }}</p>
        {% endblock %}
    </div>
...
```twig
{% props
    defaultBaseClasses = 'ct-product-card card',
    defaultVariants = {},
%}
...
❌ Don't
```twig
<div class="ct-product-card card" role="article">
    ...
</div>
```
...
* Custom properties must bring an `ct-` prefix to avoid conflicts with Bootstrap `bs-` variables.
...
.ct-product-card {
    ...
    --ct-product-card-media-height: 200px;
}

/* Avoids nesting by using BEM naming structure */
.ct-product-card__media {
    height: var(--ct-product-card-media-height);
}
...
You can add a corresponding JS/TS file for your component. Component JavaScript is built with Vite into ES modules and loaded via import map when the component is used on a page. You can use plain JS/TS or the Contena component system to add encapsulated frontend logic.

For more information you can have a look at the [README.md](../../app/frontend/src/component-system/README.md).
...
```bash
# Full Frontend JS build (includes component build + publish)
composer build:js:frontend
```

### Dev server from project root

For local development with hot reload and live component imports, start the Frontend Vite dev server:

```bash
composer frontend:dev-server
```

In dev mode, Contena loads component JS/CSS directly from the running Vite server.
Normal theme files are also compiled and served with Vite in dev-server mode, so you can work with all together.
When the dev server stops, Contena falls back to published production assets.

### What the pipeline does

1. Vite builds component JS/TS/SCSS/CSS from `Resources/views/components/` into `Resources/public/frontend/components/`.
2. `assets:install` publishes those files into `public/bundles/<bundle>/frontend/components/`.
...
A bundle can provide `Resources/app/frontend/vite.components.config.mts`. Use the shared Contena factory and only override what you need:

```ts
import path from 'node:path';
import { createComponentBuildConfig } from '../../../../../../src/Frontend/Resources/app/frontend/build/vite/component-config-factory';

export default async () => {
    const frontendAppDir = import.meta.dirname;

    return createComponentBuildConfig({
        componentRoot: path.resolve(frontendAppDir, '../../views/components'),
        outDir: path.resolve(frontendAppDir, '../../public/frontend/components'),
