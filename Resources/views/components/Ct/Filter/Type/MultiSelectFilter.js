export default class MultiSelectFilter extends ContenaComponent {

    init() {
        this.activeOptions = [];
        this.searchInput = this.el.querySelector('.ct-multi-select-filter__search');
        this.options = this.el.querySelectorAll('.ct-multi-select-filter__list-item');
        this.optionInputs = this.el.querySelectorAll('.ct-multi-select-filter__list-item-checkbox');
    ...
    registerEvents() {
        ...
        Contena.on(`Filter:Remove:${this.paramName}`, this.handleFilterRemove.bind(this));
    }
    ...
    handleSearchInput(event) {
        ...
        ({ searchTerm } = Contena.emitInterception('MultiSelectFilter:PreSearch', { searchTerm }));
        ...
        Contena.emit('MultiSelectFilter:Search', searchTerm);
    }
    ...
    handleOptionChange(event) {
        ...
        ({ paramName, value, option, label } = Contena.emitInterception('MultiSelectFilter:PreChange', { paramName, value, option, label }));
        ...
        Contena.emit('MultiSelectFilter:Change', eventData);
        Contena.emit('Filter:Change', eventData);
    ...
    setStateFromUrl() {
        ...
        this.optionInputs.forEach(input => {
            if (options.includes(input.value)) {
                ...
                Contena.emitQueued('Filter:Init', {
    ...
    destroy() {
        ...
        Contena.off(`Filter:Remove:${this.paramName}`, this.handleFilterRemove.bind(this));
    }
}
