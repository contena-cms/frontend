...
function createBlogListing() {
    ...
    // The `ContenaComponent` test double does not call `init()` from its constructor.
    const listing = new BlogListing(el);
    listing.init();
...
describe('Ct:Blog:LayoutSwitch', () => {
    ...
    it('emits the listingLayout parameter name together with the clicked layout', () => {
        ...
        expect(Contena.emit).toHaveBeenCalledWith('LayoutSwitch:Change', LISTING_LAYOUT_PARAM, 'horizontal');
    });

    // `BlogListing` writes the layout key twice from two independent option objects — once under the name
