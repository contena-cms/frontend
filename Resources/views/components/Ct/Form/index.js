const CONTROL_SELECTOR = '.ct-form-field__control';
const FEEDBACK_SELECTOR = '.ct-form-feedback';
const INVALID_CLASS = 'is-invalid';
const STATUS_SELECTOR = '.ct-form__status';
 ...
 */
export default class Form extends ContenaComponent {
    static options = {
        ajax: false,
        replaceSelectors: [],
    ...
    init() {
        ...
        this.options.replaceSelectors =
            typeof selectors === 'string' ? [selectors] : Array.isArray(selectors) ? [...selectors] : [];
    ...
    intercept(eventName, payload) {
        const result = Contena.emitInterception(eventName, payload);
    ...
    onFieldChange(event) {
        ...
        if (
            Array.isArray(this.options.submitOnChange) &&
    ...
    onSubmit(event) {
        ...
        if (invalidFields.length > 0) {
            ...
            Contena.emit('Form:ValidationFailed', { form: this.el, invalidFields });
    ...
    async submitForm(tasks, submitter) {
        try {
            ...
            const { action, method, formData } = this.intercept('Form:BeforeRequest', {
                ...
                formData: Contena.serializeForm(this.el),
            });

            Contena.emit('Form:Submit', { form: this.el, action, method, formData });

            const response = await this.sendRequest(action, method, formData);

            await this.handleResponse(response);
        } catch (error) {
            console.error('[Ct:Form]: Submission failed.', error);

            this.stopLoading();

            Contena.emit('Form:Error', { form: this.el, error });
        } finally {
            this.isSubmitting = false;
    ...
    async handleResponse(response) {
        ...
        if (!response.ok) {
            Contena.emit('Form:Error', { form: this.el, response, payload });

            return;
        }

        if (isJson) {
            Contena.emit('Form:Response', { form: this.el, payload });
        ...
        Contena.emit('Form:Response', { form: this.el, html: payload });
    }
    ...
    applyServerViolations(payload) {
        ...
        errors.forEach((error) => {
            ...
            if (feedback && error.detail) {
                ...
                message.className = 'ct-form-feedback__message invalid-feedback';
    ...
    /**
     * Falls back to plain markup, so a field contributed from outside the `Ct:Form:*` set is
     * served too.
     *
     * @returns {HTMLElement[]}
     */
