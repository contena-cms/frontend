...
// Import order is load-bearing: this module's side effect installs the `globalThis.ContenaComponent`
...
function createForm(innerHtml, options = {}, attributes = {}) {
    ...
    el.innerHTML = `
        <div class="ct-form__status visually-hidden" role="status" data-loading-text="Loading..."></div>
        ${innerHtml}`;
    document.body.appendChild(el);

    // The `ContenaComponent` test double does not call `init()` from its constructor.
...
function field(name, { violationPath = null, validation = null } = {}) {
    return `
        <div class="ct-form-input ct-form-field form-group"${violationPath ? ` data-violation-path="${violationPath}"` : ''}>
            <input class="ct-form-input__control ct-form-field__control form-control"
                   id="${name}" name="${name}" aria-describedby="${name}-feedback"
                   ${validation ? `data-validation="${validation}"` : ''}>
            <div class="ct-form-feedback form-field-feedback" id="${name}-feedback"></div>
        </div>`;
}
...
describe('Ct:Form', () => {
    ...
    beforeEach(() => {
        ...
        Contena.emitInterception.mockClear();
        Contena.emitInterception.mockImplementation((_event, payload) => payload);
        Contena.serializeForm.mockImplementation((form) => new FormData(form));
    });
    ...
    it('blocks an invalid submission and moves the focus to the first invalid field', () => {
        ...
        expect(Contena.emit).toHaveBeenCalledWith('Form:ValidationFailed', { form: el, invalidFields: [control] });
    });

    it('sends the form fields when submitting via ajax', async () => {
        const { el } = createForm(field('title'), { ajax: true });
        el.querySelector('input').value = 'Great article';
    ...
    it('takes the action and method of the submitter over the ones of the form', async () => {
        const { el } = createForm(
            `${field('title')}<button type="submit" formaction="/other-path" formmethod="GET">Save</button>`,
            { ajax: true },
        );
    ...
    it('waits for every pre-submit task and sends what they wrote in one request', async () => {
        ...
        let releaseToken;
        Contena.emitInterception.mockImplementation((event, payload) => {
    ...
    it('announces the submission once, through the status region the form renders', async () => {
        ...
        const status = el.querySelector('.ct-form__status');
        status.dataset.loadingText = 'Loading now...';

        submit(el);

        expect(status.textContent).toBe('Loading now...');
    ...
    it('keeps the nodes inside a submit button alive across the loading state', async () => {
        const { el } = createForm(`${field('title')}<button type="submit"><span class="sw-icon">icon</span>Save</button>`, {
            ajax: true,
        });
        const icon = el.querySelector('.ct-icon');

        submit(el);
        expect(el.querySelector('.ct-icon')).toBeNull();

        await vi.waitFor(() => expect(el.querySelector('button').hasAttribute('aria-disabled')).toBe(false));

        expect(el.querySelector('.ct-icon')).toBe(icon);
    });
    ...
    it('restores the submit button and reports the failure when the request fails', async () => {
        ...
        await vi.waitFor(() => expect(el.querySelector('button').hasAttribute('aria-disabled')).toBe(false));
        expect(Contena.emit).toHaveBeenCalledWith('Form:Error', expect.objectContaining({ form: el }));
    });

    it('replaces the configured fragments of the response', async () => {
        const container = document.createElement('div');
        container.className = 'js-comment-container';
        container.innerHTML = '<p>old</p>';
        document.body.appendChild(container);

        window.fetch = vi.fn(() =>
            Promise.resolve(htmlResponse('<html><body><div class="js-comment-container"><p>new</p></div></body></html>')),
        );

        const { el } = createForm(field('title'), { ajax: true, replaceSelectors: ['.js-comment-container'] });
        ...
        expect(Contena.emit).toHaveBeenCalledWith('Form:Response', expect.objectContaining({ form: el }));
    });
    ...
    it('maps the violations of an error response onto the fields that answer for them', async () => {
        ...
        await vi.waitFor(() =>
            expect(el.querySelector('#title-feedback').textContent).toContain('Input should not be empty.'),
        );
        ...
        expect(Contena.emit).toHaveBeenCalledWith('Form:ValidationFailed', expect.objectContaining({ form: el }));
    });
    ...
    it.each([
        ...
        [
            'the captcha shape',
            [{ type: 'danger', error: 'invalid_captcha', alert: '<div>Invalid</div>' }],
            403,
        ],
        ...
        await vi.waitFor(() =>
            expect(Contena.emit).toHaveBeenCalledWith('Form:Error', expect.objectContaining({ form: el, payload })),
        );

        expect(Contena.emit).not.toHaveBeenCalledWith('Form:Response', expect.anything());
        expect(el.querySelector('button').hasAttribute('aria-disabled')).toBe(false);
        expect(el.hasAttribute('aria-busy')).toBe(false);
    ...
    it('aborts the submission and cleans up when a pre-submit task rejects', async () => {
        ...
        Contena.emitInterception.mockImplementationOnce((_event, payload) => {
    ...
    it('keeps the request target out of the pre-submit payload', () => {
        ...
        expect(payloads['Form:BeforeRequest']).toEqual(
    ...
    it('leaves a fragment alone when the response carries an empty one', async () => {
        ...
        window.fetch = vi.fn(() =>
            Promise.resolve(htmlResponse('<html><body><div class="js-comment-container"></div></body></html>')),
        );
        ...
        await vi.waitFor(() => expect(Contena.emit).toHaveBeenCalledWith('Form:Response', expect.anything()));
    ...
    it('keeps an ajax form and a native form on the same page independent', async () => {
        ...
        const comment = createForm(`${field('title')}<button type="submit">Save comment</button>`, {
            ajax: true,
        ...
        expect(comment.el.querySelector('button').getAttribute('aria-disabled')).toBe('true');
        expect(container.classList.contains('has-element-loader')).toBe(true);
        ...
        await vi.waitFor(() => expect(comment.el.querySelector('button').hasAttribute('aria-disabled')).toBe(false));
