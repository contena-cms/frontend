...
function createLightboxGallery({ withImage = true, clickZoomScale = 2 } = {}) {
    document.body.innerHTML = `
        <div class="modal ct-media-gallery-lightbox">
            <section class="ct-media-gallery ct-media-gallery--is-lightbox">
                <div class="ct-media-gallery__previews">
                    <div class="ct-media-gallery__preview-item">
                        ${withImage ? '<img class="ct-media-gallery__preview" src="photo.jpg" srcset="photo-small.jpg 100w">' : '<video class="ct-media-gallery__preview"></video>'}
                    </div>
                </div>
            </section>
        </div>
    ...
    const root = document.querySelector('.ct-media-gallery');
    const container = root.querySelector('.ct-media-gallery__preview-item');
    const imageEl = container.querySelector(':scope > img');

    const component = new MediaGallery(root, {
        isLightbox: true,
        showMagnifier: false,
        clickZoomScale,
    });
...
describe('Ct:Media:Gallery lightbox click zoom', () => {
    afterEach(() => {
        vi.restoreAllMocks();
        document.body.innerHTML = '';
    });
