...
describe('ct-theme-list-item', () => {
    async function createWrapper(props = {}) {
        const component = await Contena.Component.build('ct-theme-list-item');
    ...
    it('falls back to default theme asset when preview media is missing', async () => {
        const assetSpy = jest.spyOn(Contena.Filter, 'getByName').mockReturnValue(() => 'preview.jpg');
        const wrapper = await createWrapper();
    ...
    it('detects active state from channel assignment or active prop', async () => {
        const wrapperWithChannel = await createWrapper({
            theme: { channels: [{ id: 'channel-id' }] },
        });
