...
function createService(client = null, loginService = null) {
    ...
    if (!loginService) {
        loginService = createLoginService(client, Contena.Context.api);
    }
...
describe('ThemeApiService', () => {
    beforeEach(() => {
        Contena.Store.get('session').languageId = 'en-GB';
    });

    it('assigns theme to a channel', async () => {
        const service = createService();
        const postSpy = jest.spyOn(service.httpClient, 'post').mockResolvedValue({ data: { foo: 'bar' } });

        const result = await service.assignTheme('theme-id', 'channel-id', { foo: 'bar' }, { baz: 'qux' });

        expect(postSpy).toHaveBeenCalledWith(
            '/_action/theme/theme-id/assign/channel-id',
            {},
