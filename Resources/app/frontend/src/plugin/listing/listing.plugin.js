...
export default class ListingPlugin extends Plugin {
    ...
    static options = {
        ...
        contentBlogListingSelector: '.content-blog-listing',
        contentBlogListingWrapperSelector: '.content-blog-listing-wrapper',
        contentBlogListingResultsSelector: '.js-listing-wrapper',
    ...
    init() {
        ...
        this._contentBlogListingWrapper = document.querySelector(this.options.contentBlogListingWrapperSelector);
        this._contentBlogListingWrapperActive = !!this._contentBlogListingWrapper;
     ...
     * under `Core/Content/Blog/Channel/Listing/Processor/` on the PHP side. Pipe-joining them
     * produces invalid queries like `p=1|2` (400 responses on `/content/category/*`). Keep this
     * in sync when the backend adds new single-valued listing params.
     *
     * @private
     */
    _mapFilters(filters) {
    ...
    _scrollTopOfListing() {
        const elemRect = this._contentBlogListingWrapper.getBoundingClientRect();
        if (elemRect.top >= 0) {
            return;
        }
    ...
    /**
     * Add classes to add loading styling for blog listing
     */
    addLoadingElementLoaderClass() {
        this._contentBlogListingWrapper.classList.add(this.options.loadingElementLoaderClass);
    }

    /**
     * Remove loading styling classes for blog listing
     */
    removeLoadingElementLoaderClass() {
        this._contentBlogListingWrapper.classList.remove(this.options.loadingElementLoaderClass);
    }

    /**
     * Send request to get filtered blog data.
     *
    ...
    sendDataRequest(filterParams) {
        ...
        if (this._contentBlogListingWrapperActive) {
    ...
    /**
     * Inject the HTML of the filtered blogs to the page.
     *
     * @param {String} response - HTML of filtered blog data.
     */
    renderResponse(response) {
        ElementReplaceHelper.replaceFromMarkup(response, this.options.contentBlogListingSelector);
    ...
    _updateAriaLive() {
        ...
        const listingResultsEl = this.el.querySelector(this.options.contentBlogListingResultsSelector);
