...
const BLOG_SEARCH_PERFORMED_EVENT = 'blog:search-performed';
const BLOG_SEARCH_SUGGESTION_SHOWN_EVENT = 'blog:search-suggestion-shown';
const BLOG_SEARCH_SUGGESTION_BLOG_VIEWED_EVENT = 'blog:search-suggestion-blog-viewed';
...
export default class SearchWidgetPlugin extends Plugin {
    ...
    _handleSearchEvent(event) {
        ...
        document.dispatchEvent(new CustomEvent(BLOG_SEARCH_PERFORMED_EVENT, {
            detail: { term: value },
        }));
    }
    ...
    _suggest(value) {
        ...
        fetch(url, {
            ...
            .then(content => {
                ...
                document.dispatchEvent(new CustomEvent(BLOG_SEARCH_SUGGESTION_SHOWN_EVENT, {
                    detail: { term: value },
                }));
     ...
     * blog:search-performed event when the user clicks the "show all
     * results" link, or a blog:search-suggestion-blog-viewed event
     * when the user clicks a blog result.
     * @param {Event} event
     * @private
     */
    _handleSuggestResultClick(event) {
        ...
        const eventName = event.target.closest('.search-suggest-total')
            ? BLOG_SEARCH_PERFORMED_EVENT
            : BLOG_SEARCH_SUGGESTION_BLOG_VIEWED_EVENT;
