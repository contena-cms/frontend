...
3. **Better events instead of overrides**
    The current override technique of the plugin system was not reintroduced to the component system, as it showed some major flaws, as overrides could only happen once which can lead to conflicts between different Contena extensions. Instead there is a central event system which is easier to use and offers a more robust public interface. In addition, it offers special interception events, for example, to manipulate request data before it is sent.

## Overview

The JavaScript component system consists of two main parts:

1. **Contena** - A new global singleton which acts as a central entry point.
2. **ContenaComponent** - The abstract base class for all components.

## Component Architecture

### Base Component Class

All components extend from the `ContenaComponent` base class, which provides:
...
### Global Contena Instance

The `Contena` class acts as a singleton that:
...
The component has to extend the `ContenaComponent` class, which is globally available. The name of the component class does not have to follow a particular pattern, but the name of the script file should have the same name as your Twig component and should be located right beside the template file.

```javascript
export default class MyComponent extends ContenaComponent {
...
Contena generates an importmap for all components based on the Twig component tag name. On initialization, Contena will search for all elements with a `data-component` attribute and will try to load the corresponding script file, if necessary. Just make sure to add the data attribute, including the tag name of your Twig component, to the root element of your component.

```Twig
<div data-component="MyComponentNamespace:MyComponent"></div>
```

When the script is loaded, Contena will automatically initialize the component class on all elements matching the selector. This also applies to elements that might be added later. You do not need to do this manually. Contena will observe the DOM tree and initialize components also on elements that are dynamically added to the document.
...
When the dev server is running, Contena serves component JS/CSS from the Vite server. When it is stopped, Contena falls back to published production assets.
...
To react to actions from other components, there is a new central event system available which can be accessed via the global `window.Contena` singleton.
...
export default class MyComponent extends ContenaComponent {
    ...
    doSomething() {
        ...
        window.Contena.emit('MyComponent:DoSomething', message);
    }
}
```
...
export default class SomeOtherComponent extends ContenaComponent {

    init() {
        window.Contena.on('MyComponent:DoSomething', (message) => {
...
export default class BuyButton extends ContenaComponent {
    ...
    onFormSubmit(event) {
        ...
        let formData = window.Contena.serializeForm(this.el);

        ({ requestUrl, formData } = window.Contena.emitInterception('BuyButton:PreSubmit', { requestUrl, formData }));

        window.Contena.emit('BuyButton:Submit', requestUrl, formData);
...
window.Contena.intercept('BuyButton:PreSubmit', (data) => {
...
Contena.callMethod('MyComponentNamespace:MyComponent', 'refresh');

// Get all instances of a component
const instances = Contena.getComponentInstances('MyComponentNamespace:MyComponent');

// Get a specific instance by element
const instance = Contena.getComponentInstanceByElement('MyComponentNamespace:MyComponent', element);
```
...
```javascript
class ReactiveComponent extends ContenaComponent {
    init() {
