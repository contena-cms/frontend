...
function addEmbed({ cookieName = 'vimeo-video', videoUrl = 'https://player.vimeo.com/video/1', videoTitle = 'Video' } = {}) {
    ...
    wrapper.className = 'ct-media-embed-video';
    wrapper.innerHTML = `
        <div class="ct-media-embed-video__consent">
            <div class="ct-media-embed-video__backdrop">
                <button type="button" class="ct-media-embed-video__accept">Accept</button>
            </div>
        </div>
    `;
    document.body.appendChild(wrapper);

    const consentEl = wrapper.querySelector('.ct-media-embed-video__consent');
...
describe('views/components/CT/Media/EmbedVideo', () => {
    ...
    test('is a ContenaComponent', () => {
        addEmbed();

        expect(components[0]).toBeInstanceOf(window.ContenaComponent);
    });
    ...
    test('replaces the placeholder with the iframe immediately when consent already exists', () => {
        ...
        const iframe = wrapper.querySelector('iframe.ct-media-embed-video__video');
        ...
        expect(wrapper.querySelector('.ct-media-embed-video__consent')).toBeNull();
    });

    test('keeps the consent placeholder gated when no consent is given yet', () => {
        const wrapper = addEmbed();

        expect(wrapper.querySelector('.ct-media-embed-video__consent')).not.toBeNull();
    ...
    test('sets the consent cookie and shows the iframe when the accept button is clicked', () => {
        ...
        wrapper.querySelector('.ct-media-embed-video__accept').click();

        expect(document.cookie).toContain('vimeo-video=1');
        expect(wrapper.querySelector('iframe.ct-media-embed-video__video').getAttribute('src'))
    ...
    test('accepting one embed replaces every same-platform embed with its own iframe, leaving others gated', () => {
        ...
        vimeoA.querySelector('.ct-media-embed-video__accept').click();

        // Both Vimeo embeds are replaced, each with its OWN video url.
        expect(vimeoA.querySelector('iframe.ct-media-embed-video__video').getAttribute('src'))
            .toBe('https://player.vimeo.com/video/a');
        expect(vimeoB.querySelector('iframe.ct-media-embed-video__video').getAttribute('src'))
        ...
        expect(youtube.querySelector('.ct-media-embed-video__consent')).not.toBeNull();
    });
    ...
    test('shows the iframe on a cookie-configuration update once the cookie is present', () => {
        ...
        document.cookie = 'vimeo-video=1;path=/';
        document.dispatchEvent(new CustomEvent('CookieConfiguration_Update'));
        expect(wrapper.querySelector('iframe.ct-media-embed-video__video')).not.toBeNull();
    });
