...
describe('ListingPlugin tests', () => {
        ...
        document.body.innerHTML = `
            ...
            <div class="content-blog-listing-wrapper" data-listing="true">
                <div class="content-blog-listing">
                    <div class="row cms-listing-row js-listing-wrapper" data-aria-live-text="Showing 24 out of 1000 articles.">
    ...
    test('should autoscroll to top with scrollOffset because we are not at the top', async () => {
        ...
        listingPlugin._contentBlogListingWrapper.getBoundingClientRect = () => ({
            top: -500,
        });
    ...
    test('should autoscroll to top of contentBlogListingWrapper because we are not at the top', () => {
        global.fetch = jest.fn(() =>
            Promise.resolve({
                ok: true,
                text: () => Promise.resolve('Listing result HTML'),
    ...
    test('_onWindowPopstate get called when browser back', () => {
        ...
        const contentBlogListingWrapper = document.createElement('div');
        contentBlogListingWrapper.classList.add('content-blog-listing-wrapper');

        document.body.append(contentBlogListingWrapper);
    ...
    test('updates the aria-live section after blog results have changed',async () => {
        ...
        global.fetch = jest.fn(() =>
            Promise.resolve({
                ...
                <div class="content-blog-listing-wrapper" data-listing="true">
                    <div class="content-blog-listing">
                        <div class="row cms-listing-row js-listing-wrapper" data-aria-live-text="Showing 2 articles.">
                            <div class="card blog-card box-standard"></div>
                            <div class="card blog-card box-standard"></div>
                        </div>
                    </div>
        ...
        // Verify that the new blog results contain the data attribute with the updated aria-live text
        expect(document.querySelector('.js-listing-wrapper').dataset.ariaLiveText).toBe('Showing 2 articles.');

        // Verify that the aria-live text in the filter panel has been updated
        expect(document.querySelector('.filter-panel-aria-live').textContent).toBe('Showing 2 articles.');
    });
    ...
    describe('_fetchValuesOfRegisteredFilters', () => {
        ...
        test('normalizes scalars, arrays and objects into a single array per key', () => {
            const scalarFilter = { getValues: () => ({ category: 'abc' }) };
            const arrayFilter = { getValues: () => ({ category: ['def', 'ghi'] }) };
            const objectFilter = { getValues: () => ({ tags: { a: 'p1', b: 'p2' } }) };

            listingPlugin._registry = [scalarFilter, arrayFilter, objectFilter];

            const result = listingPlugin._fetchValuesOfRegisteredFilters();

            expect(result.category).toEqual(['abc', 'def', 'ghi']);
            expect(result.tags).toEqual(['p1', 'p2']);
        });

        test('skips filters without getValues and handles null values gracefully', () => {
            const brokenFilter = {};
            const nullReturning = { getValues: () => null };
            const withNullValue = { getValues: () => ({ category: null, featured: undefined }) };
            const validFilter = { getValues: () => ({ category: ['abc'] }) };
    ...
    describe('_mapFilters', () => {
        test('returns single-valued keys like `p` as scalars even when multiple values were collected', () => {
            const mapped = listingPlugin._mapFilters({
                p: [1, 2],
                order: ['name-asc', 'created-at-desc'],
                limit: [24, 48],
            });

            expect(mapped.p).toBe('2');
            expect(mapped.order).toBe('created-at-desc');
        ...
        test('pipe-joins multi-valued filter keys like `category` and `tags`', () => {
            const mapped = listingPlugin._mapFilters({
                category: ['abc', 'def'],
                tags: ['p1', 'p2', 'p3'],
            });

            expect(mapped.category).toBe('abc|def');
            expect(mapped.tags).toBe('p1|p2|p3');
        });
        ...
        test('omits empty and nullish values', () => {
            ...
            expect(mapped.featured).toBeUndefined();
    ...
    test('builds the labels for the active filters and renders them inside the filter panel', async () => {
        ...
        const MockBooleanFilter = {
            getLabels: () => [{ label: 'Featured', id: 'featured' }],
            getValues: () => { return { 'featured': '1' }; },
        };
        ...
        const MockMultiSelectFilter = {
            ...
            getValues: () => { return { 'category': ['0190da2684cb710aac3d3291a340b3e3', '0190da2684cb710aac3d32919db761bb'] }; },
        };
        ...
        expect(activeFilterElements[0].textContent).toMatch('Featured');
        expect(activeFilterElements[0].getAttribute('aria-label')).toBe('Remove filter: Featured');
    ...
    test('redirects to login on forbidden listing request', async () => {
        ...
        window.activeRoute = 'frontend.member.profile.page';
        window.activeRouteParameters = JSON.stringify({ profileId: 'test-profile' });
        ...
        expect(loginUrl.searchParams.get('redirectTo')).toBe('frontend.member.profile.page');
