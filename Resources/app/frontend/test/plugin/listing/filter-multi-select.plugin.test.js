...
describe('FilterMultiSelect tests', () => {
    ...
    beforeEach(() => {
        ...
        mockElement.appendChild(contentBlogListingWrapper);
    ...
    test('should render correct counter depending on current selection', () => {
        document.body.innerHTML = `
            <div class="filter-multi-select filter-multi-select-category filter-panel-item dropdown" role="listitem" data-filter-multi-select="true">
                <button class="filter-panel-item-toggle btn" aria-expanded="false" aria-label="Filter by category" data-bs-toggle="dropdown" data-boundary="viewport" data-bs-offset="0,8" aria-haspopup="true">
                    Category <span class="filter-multi-select-count"></span>
                </button>
    
                <div class="filter-multi-select-dropdown filter-panel-item-dropdown dropdown-menu" id="filter-category-1854714896">
        ...
        expect(document.querySelector('.filter-panel-item-toggle').getAttribute('aria-label')).toBe('Filter by category (1 selected)');
    });
});
