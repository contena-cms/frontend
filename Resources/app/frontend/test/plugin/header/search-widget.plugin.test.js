...
describe('SearchPlugin Tests', () => {
    ...
    test('_handleSearchEvent dispatches blog:search-performed event on form submit when term is valid', () => {
        const listener = jest.fn();
        document.addEventListener('blog:search-performed', listener);
        ...
        document.removeEventListener('blog:search-performed', listener);
    });
    ...
    test('_handleSuggestResultClick dispatches blog:search-suggestion-blog-viewed when clicking a blog link', () => {
        const listener = jest.fn();
        document.addEventListener('blog:search-suggestion-blog-viewed', listener);
        searchPlugin._inputField.value = 'shoes';

        const link = document.createElement('a');
        link.setAttribute('href', '/blog/123');
        ...
        document.removeEventListener('blog:search-suggestion-blog-viewed', listener);
    });

    test('_handleSuggestResultClick dispatches blog:search-performed when clicking the show-all-results link', () => {
        const listener = jest.fn();
        document.addEventListener('blog:search-performed', listener);
        searchPlugin._inputField.value = 'shoes';
    ...
    test('_handleSuggestResultClick ignores clicks that are not on a link', () => {
        const blogListener = jest.fn();
        const performedListener = jest.fn();
        document.addEventListener('blog:search-suggestion-blog-viewed', blogListener);
        document.addEventListener('blog:search-performed', performedListener);
        searchPlugin._inputField.value = 'shoes';
        ...
        expect(blogListener).not.toHaveBeenCalled();
        expect(performedListener).not.toHaveBeenCalled();

        document.removeEventListener('blog:search-suggestion-blog-viewed', blogListener);
        document.removeEventListener('blog:search-performed', performedListener);
    });
    ...
    test('_suggest should handle successful AJAX request', async () => {
        ...
        document.addEventListener('blog:search-suggestion-shown', suggestionListener);
        ...
        document.removeEventListener('blog:search-suggestion-shown', suggestionListener);
    });
