...
// two levels above the app/frontend package root.
 ...
 * var/plugins.json. Falls back to the core Frontend path only when the
 ...
 * `Resources/app/frontend/node_modules` so the IDE can resolve npm
    ...
    return mergeConfig(
        {
            ...
            resolve: {
                // In tests, resolve the 'contena' bare specifier to the manual mock so
                // both import-style and window-global-style components work without any
                // real contena.js on disk.  The mock also assigns to globalThis so
                // legacy components that do `({ Contena } = window)` still work.
                alias: {
                    contena: path.resolve(configDir, '__mocks__/contena.ts'),
                },
            },
            server: {
                fs: {
                    // Allow Vite to serve files from every component root, which may be
                    // outside the app/frontend package directory.
