...
/**
 * `global.default` always lives in the core Frontend - the rule's home. Candidate snippets and the
 * allow list are read from the directory ESLint runs in (`context.cwd`, optionally narrowed by the
 * `snippetRoot` option), so the rule also covers frontend snippets shipped by other bundles when
 * they are linted from their own directory - without the core Frontend referencing those bundles.
 */
const GLOBAL_DEFAULT_DIR = path.resolve(FRONTEND_ROOT, '..', '..', 'snippet');
...
function getDuplicates(cwd, snippetRoot) {
    ...
    // global.default lives in the core Frontend (the rule's home).
    ...
    for (const file of fs.existsSync(GLOBAL_DEFAULT_DIR) ? fs.readdirSync(GLOBAL_DEFAULT_DIR) : []) {
        const match = /^frontend\.([a-zA-Z-]+)\.json$/.exec(file);
        if (!match) {
            continue;
        }
    ...
    // Candidate frontend snippets, grouped by file name (`<name>.<locale>.json` -> one group), flattened per locale.
    ...
    meta: {
        ...
        docs: {
            description: 'Disallow snippet values that are identical to a `global.default` translation in every locale; '
                ...
                + '`global.default` keys outside the core Frontend snippets.',
            recommended: true,
        },
        schema: [
            {
        ...
        messages: {
            ...
            noGlobalDefaultDefinition: 'Snippet Key `{{key}}` is defined in the reserved `global.default` namespace. '
                + 'Only the core Frontend snippets (`Resources/snippet/frontend.*.json`) may define '
