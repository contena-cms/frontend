 ...
 * Resources/app/frontend/node_modules/.  Node's normal upward-scan
...
export function extensionModuleResolverPlugin(projectRoot: string): Plugin {
    ...
    if (fs.existsSync(pluginsJson)) {
        ...
        for (const bundle of bundles) {
            ...
            const frontendAppDir = path.join(resolveBundleBasePath(bundle.basePath), bundle.frontend.path, '..');
            extensionResolvers.push({
                componentRoot,
                nodeModulesPath: path.join(frontendAppDir, 'node_modules'),
                resolveFromExtension: createRequire(path.join(frontendAppDir, 'package.json')).resolve,
            });
        }
    }
