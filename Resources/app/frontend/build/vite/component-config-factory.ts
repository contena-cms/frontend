...
const PLAIN_CSS_SHIM_PREFIX = '\0ct-plain-css:';
...
function extensionNodeModulesPlugin(frontendAppDir: string): Plugin {
    const resolveFromExtension = createRequire(path.join(frontendAppDir, 'package.json'));
...
/**
 * Builds a reusable Vite config for Frontend components.
 *
 * Use this in both Contena's generic build config and custom extension config
...
export async function createComponentBuildConfig(options: ComponentBuildConfigOptions): Promise<UserConfig> {
    const {
        ...
        namespace,
        frontendAppDir,
        coreFrontendAppDir = path.resolve(import.meta.dirname, '../..'),
    ...
    const isExtension = namespace !== 'Frontend';
    ...
    const pluginStack: Plugin[] = [
        ...
        extensionNodeModulesPlugin(frontendAppDir),
        componentMapPlugin(),
        plainCssShimPlugin(plainCssShims),
    ];

    const allExternals = Array.from(new Set(['contena', ...external]));

    const bundleAssetDir = toAssetDirectory(namespace);

    return {
        base: `/bundles/${bundleAssetDir}/frontend/components/`,
        css: {
            preprocessorOptions: {
                scss: {
