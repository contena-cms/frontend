...
const COMP_CSS_PREFIX = '/__ct-comp-css/';
 ...
 * (`var/cache/frontend_components.dev.json`).
 *
 ...
 *              ct-theme-scss plugin. PHP uses these for `<link>` tags
 ...
 *              modules. The first entry is always the core frontend
 *              main.js; any plugin bundles with a frontend entryFilePath
 ...
 *              this against the current channel theme to decide whether
 ...
 *
 * When the dev server stops the file is removed and the Frontend
 * transparently falls back to the production import map and compiled CSS.
 */
/**
 * @param projectRoot  Absolute path to the Contena project root.
...
export function devImportMapPlugin(projectRoot: string, scssLoadPaths: string[] = []): Plugin {
    const flagFile = path.join(projectRoot, 'var/cache/frontend_components.dev.json');
    let viteRoot = '';
    ...
    return {
        ...
        configureServer(server: ViteDevServer) {
            ...
            // resolve a URL like "/__ct-comp-css/ComponentTestApp/Wusel/Dusel.scss"
            ...
            const componentBundles: ComponentBundleDefinition[] = Object.entries(bundles).map(([bundleName, bundle]) => {
                const namespace = bundleName === 'Frontend' ? '' : bundleName;

                return {
                    bundleName,
                    namespace,
                    importNamespace: bundleName === 'Frontend' ? undefined : bundleName,
            ...
            // /__ct-comp-css/ middleware, not through Vite's module graph, so
            ...
            const sassLogger = {
                warn(message: string, options?: { deprecation?: boolean }): void {
                    ...
                    server.config.logger.warn(`[ct-dev-import-map][sass] ${message}`, { timestamp: true });
                },
            };
            ...
            const write = (): void => {
                ...
                // contena runtime module — lives inside the Vite root so it
                // gets a clean URL without the /@fs/ prefix.
                const contenaSrc = path.join(viteRoot, 'src/contena.ts');
                if (fs.existsSync(contenaSrc)) {
                    imports['contena'] = `${origin}/src/contena.ts`;
                }
                ...
                // 1. Main theme SCSS (ct-theme-scss middleware) — when theme-files.json exists.
                // 2. Individual component SCSS files — served on-the-fly by the
                //    /__ct-comp-css/ middleware above, mirroring the per-file CSS
                ...
                // Core frontend main.js lives inside the Vite root so it gets a
                ...
                // with a frontend.entryFilePath are included, and technicalName
                // 'frontend' is the core entry, not a plugin.
                const scripts: string[] = [];
                if (fs.existsSync(pluginsJsonPath)) {
                    // Core frontend entry is always first.
                    scripts.push(`${origin}/src/main.js`);

                    for (const [, bundle] of Object.entries(bundles)) {
                        const entryFilePath = bundle.frontend?.entryFilePath;
                        if (!entryFilePath || bundle.technicalName === 'frontend') continue;
                ...
                server.config.logger.info(
                    `[ct-dev-import-map] dev flag file written → ${flagFile}`,
                    { timestamp: true },
                );
            };
            ...
            const onStyleEvent = (event: 'changed' | 'added' | 'removed', file: string): void => {
                ...
                const notify = (): void => {
                    server.config.logger.info(
                        `[ct-dev-import-map] component style ${event}: ${path.relative(projectRoot, file)} → full reload`,
                        { timestamp: true },
                    );
