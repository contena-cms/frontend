...
const { getNpmInstallCommand, getScssLoadPaths, shouldAllowInstallScripts, shouldInstallNpmDependencies } = require('./build-components.js') as {
    ...
    getScssLoadPaths: (frontendAppDir: string, coreFrontendAppDir: string) => string[];
...
describe('build-components scss load paths', () => {
    const extensionDir = path.join('/ext', 'Resources', 'app', 'frontend');
    const coreDir = path.join('/core', 'Resources', 'app', 'frontend');
...
describe('build-components npm install policy', () => {
    ...
    it('disables scripts by default even when lockfile is present', () => {
        const frontendAppDir = path.join(fixtureRoot, 'with-lock');
        fs.mkdirSync(frontendAppDir, { recursive: true });
        fs.writeFileSync(path.join(frontendAppDir, 'package-lock.json'), '{}');

        const result = getNpmInstallCommand(frontendAppDir, {});
    ...
    it('falls back to npm install and still disables scripts without a lockfile', () => {
        const frontendAppDir = path.join(fixtureRoot, 'no-lock');
        fs.mkdirSync(frontendAppDir, { recursive: true });
    ...
    it('allows lifecycle scripts only when explicitly opted in', () => {
        const frontendAppDir = path.join(fixtureRoot, 'opt-in');
        fs.mkdirSync(frontendAppDir, { recursive: true });
        fs.writeFileSync(path.join(frontendAppDir, 'package-lock.json'), '{}');
    ...
    it('skips dependency install when node_modules already exists', () => {
        const frontendAppDir = path.join(fixtureRoot, 'with-node-modules');
        fs.mkdirSync(path.join(frontendAppDir, 'node_modules'), { recursive: true });

        expect(shouldInstallNpmDependencies(frontendAppDir, {})).toBe(false);
    });

    it('installs dependencies when node_modules is missing', () => {
        const frontendAppDir = path.join(fixtureRoot, 'without-node-modules');
        fs.mkdirSync(frontendAppDir, { recursive: true });

        expect(shouldInstallNpmDependencies(frontendAppDir, {})).toBe(true);
    });

    it('allows forcing dependency install via env', () => {
        const frontendAppDir = path.join(fixtureRoot, 'forced-install');
        fs.mkdirSync(path.join(frontendAppDir, 'node_modules'), { recursive: true });

        expect(shouldInstallNpmDependencies(frontendAppDir, { FORCE_COMPONENT_DEP_INSTALL: '1' })).toBe(true);
    });
});
