...
describe('extensionModuleResolverPlugin', () => {
    ...
    it('resolves bare package imports from extension node_modules', () => {
        ...
        const frontendSrc = path.join(fixtureRoot, extensionBase, 'Resources/app/frontend/src');
        const packageRoot = path.join(fixtureRoot, extensionBase, 'Resources/app/frontend/node_modules/test-package');
        ...
        fs.writeFileSync(path.join(fixtureRoot, 'var/plugins.json'), JSON.stringify({
            TestPlugin: {
                ...
                frontend: { path: 'Resources/app/frontend/src' },
            },
        }));
    ...
    it('resolves imports when plugins.json basePath is absolute', () => {
        ...
        const frontendSrc = path.join(extensionBase, 'Resources/app/frontend/src');
        const packageRoot = path.join(extensionBase, 'Resources/app/frontend/node_modules/test-abs-package');
    ...
    it('prefers exports.import over require/main entries', () => {
        ...
        const componentsRoot = path.join(fixtureRoot, extensionBase, 'Resources/views/components');
        const frontendSrc = path.join(fixtureRoot, extensionBase, 'Resources/app/frontend/src');
        const packageRoot = path.join(fixtureRoot, extensionBase, 'Resources/app/frontend/node_modules/test-import-condition');
    ...
    it('does not resolve for non-component importers', () => {
        ...
        const resolved = resolveId?.call({}, 'test-package', path.join(fixtureRoot, 'src/Frontend/Resources/app/frontend/src/main.js'), { isEntry: false });
