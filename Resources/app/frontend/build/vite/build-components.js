 ...
 *  - If the bundle provides `Resources/app/frontend/vite.components.config.mts`,
...
function getNpmInstallCommand(frontendAppDir, env = process.env) {
    const hasPackageLock = fs.existsSync(path.join(frontendAppDir, 'package-lock.json'));
...
/**
 * SCSS load paths for a component build: the extension's own vendor first, then core Frontend
 ...
 * after the frontend's postinstall hook has run. Load paths are what sass supports for this, so
 * the build must not depend on that symlink.
 */
function getScssLoadPaths(frontendAppDir, coreFrontendAppDir) {
    return [
        path.join(frontendAppDir, 'vendor'),
        path.join(coreFrontendAppDir, 'vendor'),
        path.join(coreFrontendAppDir, 'src/scss'),
        path.join(frontendAppDir, 'node_modules'),
        path.join(coreFrontendAppDir, 'node_modules'),
    ];
}

function shouldInstallNpmDependencies(frontendAppDir, env = process.env) {
    if (env.FORCE_COMPONENT_DEP_INSTALL === '1') {
        return true;
    }

    return !fs.existsSync(path.join(frontendAppDir, 'node_modules'));
}
 ...
 * installed into Resources/app/frontend/node_modules/.  These are sibling
 ...
 * the frontend app directory.
 *
 * Only applied for extension bundles (non-Frontend) that have their own
 * frontend app directory.
 */
function extensionNodeModulesPlugin(frontendAppDir) {
    const resolve = createRequire(path.join(frontendAppDir, 'package.json'));
    return {
...
async function main() {
    ...
    await Promise.all(bundlesWithComponents.map(async ([bundleName, config]) => {
        ...
        const frontendAppDir = path.join(bundleAbsPath, 'Resources', 'app', 'frontend');
        const componentRoot = path.join(bundleAbsPath, COMPONENTS_PATH);
        const outDir = path.join(bundleAbsPath, 'Resources', 'public', 'frontend', 'components');
        const customConfigMts = path.join(frontendAppDir, 'vite.components.config.mts');
        ...
        // (JS, TS, SCSS, or plain CSS).  This covers the core Frontend bundle
        ...
        if (fs.existsSync(path.join(frontendAppDir, 'package.json'))) {
            if (shouldInstallNpmDependencies(frontendAppDir)) {
                const npmInstall = getNpmInstallCommand(frontendAppDir);
                const scriptPolicy = npmInstall.scriptsAllowed
                    ? 'lifecycle scripts enabled'
                    : 'lifecycle scripts disabled';

                console.log(`  ${npmInstall.cmd} ${npmInstall.args.join(' ')} in ${frontendAppDir} (${scriptPolicy})`);
                await spawnAsync(npmInstall.cmd, npmInstall.args, { cwd: frontendAppDir });
            } else {
                console.log(`  skipping npm install in ${frontendAppDir} (node_modules already present)`);
            }
        }

        // Bundle with a custom Vite config — honour it via configFile.
        if (customConfig !== null) {
            await build({ configFile: customConfig, root: frontendAppDir });
            return;
        }

        // Generic build: config constructed inline — no env var sharing across
        // parallel builds.
        const isExtension = bundleName !== 'Frontend';
        const namespace = bundleName;

        // Core Frontend's app/frontend directory — used as vendor fallback for extensions.
        const coreFrontendAppDir = path.join(scriptDir, '../..');

        const scssLoadPaths = getScssLoadPaths(frontendAppDir, coreFrontendAppDir);

        // Build the combined entry map.
        // For extensions the entry name carries the namespace prefix so the
        // Resources/public/frontend/components/ tree can be served without path rewriting.
        ...
        const plainCssShimPrefix = '\0ct-plain-css:';
        ...
        await build({
            ...
            plugins: [
                ...(isExtension ? [extensionNodeModulesPlugin(frontendAppDir)] : []),
                componentMapPlugin(),
                plainCssShimPlugin,
            ],
        });
