...
describe('devImportMapPlugin', () => {
    ...
    it('writes dev map with component imports and keeps running when theme json is malformed', async () => {
        const viteRoot = path.join(fixtureRoot, 'src/Frontend/Resources/app/frontend');
        ...
        const flagFile = path.join(fixtureRoot, 'var/cache/frontend_components.dev.json');
        await waitUntil(() => {
    ...
    it('serves component css files via middleware and rewrites map before add-triggered reload', async () => {
        ...
        const viteRoot = path.join(pluginRoot, 'src/Frontend/Resources/app/frontend');
        ...
        const flagFile = path.join(pluginRoot, 'var/cache/frontend_components.dev.json');
        await waitUntil(() => {
            expect(fs.existsSync(flagFile)).toBe(true);
        });
        ...
        const cssMiddleware = middlewareEntries.find(entry => entry.prefix === '/__ct-comp-css/');
        ...
        await waitUntil(() => {
            ...
            expect(map.styles).toContain('http://localhost:5181/__ct-comp-css/TestPlugin/Foo/Baz.css');
        });
    });
