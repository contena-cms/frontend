...
/**
 * Absolute path to the Contena project root.
 */
...
function resolveFrontendBasePath(): string {
    ...
    if (fs.existsSync(pluginsFile)) {
        try {
            ...
            const frontendBasePath = bundles.Frontend?.basePath;

            if (typeof frontendBasePath === 'string' && frontendBasePath !== '') {
                return path.resolve(projectRoot, frontendBasePath);
            }
        } catch {
    ...
    return path.resolve(projectRoot, 'src/Frontend');
}

/**
 * Absolute path to the Frontend core component root.
 */
export const componentRoot = path.resolve(resolveFrontendBasePath(), COMPONENTS_PATH);
...
export async function buildComponentEntries(): Promise<Record<string, string>> {
    const files = await glob('**/*.{js,ts}', {
        cwd: componentRoot,
        // The `node_modules` exclude is required because the frontend's
        // postinstall hook symlinks `views/components/node_modules` to the
        // frontend app's `node_modules` so IDEs and tsc can resolve bare
 ...
 */
export const PLAIN_CSS_SHIM_PREFIX = '\0ct-plain-css:';
