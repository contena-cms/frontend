 ...
 *    `@contena-ag/dive/quickview` from a component file.
 ...
 *
 * The symlink is re-created on every `npm install` via the frontend's
...
// scriptDir = Resources/app/frontend/build
const frontendAppDir = path.resolve(scriptDir, '..');
const componentsDir = path.resolve(frontendAppDir, '../../views/components');
const linkPath = path.join(componentsDir, 'node_modules');
const linkTarget = path.relative(componentsDir, path.join(frontendAppDir, 'node_modules'));
