...
readonly class ThemeScripts
{
     ...
     *
     * The file lives at `cache/frontend_components.dev.json` within the
     * `contena.filesystem.temp` filesystem (rooted at `var/`).
     *
     * Structure written by dev-import-map-plugin:
     *   imports  — ES module import map (component tags → dev-server URLs)
     *   styles   — ordered CSS URLs served by the ct-theme-scss middleware
     *
    ...
    public function getDevImportMap(): ?array
    {
        ...
        $flagPath = 'cache/frontend_components.dev.json';
        ...
        try {
            ...
            if ($requestThemeId !== null && $requestThemeId !== $devThemeId) {
                $this->logger->debug('Frontend dev import map skipped due to theme mismatch.', [
            ...
            $this->logger->warning('Invalid frontend dev import map; skipping dev import map integration.', [
                'path' => $flagPath,
                'exception' => $exception,
            ]);
