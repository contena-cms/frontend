...
class ThemeCompiler implements ThemeCompilerInterface
{
    ...
    public function buildComponentImportMap(
        ...
        $coreVendorMap = $this->readBundleBuildMeta('Frontend', $configurationCollection)['vendorMap'] ?? [];
        foreach ($coreVendorMap as $specifier => $chunkPath) {
            $imports[$specifier] = '/bundles/' . $this->toAssetDirectory('Frontend') . '/frontend/components/' . $chunkPath;
        }

        // The contena singleton is published as a normal bundle asset.
        $imports['contena'] = '/bundles/' . $this->toAssetDirectory('Frontend') . '/frontend/contena/contena.js';

        // Component entries (with content-hashed filenames) come from per-bundle
        // build metadata in `public/bundles/<bundle>/frontend/components/.vite/build-meta.json`.
    ...
    private function readBundleComponentManifest(
        ...
        $result = [];
        $publicBase = '/bundles/' . $this->toAssetDirectory($bundleName) . '/frontend/components/';
    ...
    private function buildExtensionVendorScopes(
        ...
        foreach ($bundleNames as $bundleName) {
            ...
            $bundleComponentsBase = '/bundles/' . $this->toAssetDirectory($bundleName) . '/frontend/components/';
     ...
     * @return list<string>
     */
    private function resolveBundleNames(?FrontendPluginConfigurationCollection $configurationCollection): array
    {
        if ($configurationCollection === null) {
            return ['Frontend'];
        }

        $bundleNames = ['Frontend'];
    ...
    {
        return 'bundles/' . $this->toAssetDirectory($bundleName) . '/frontend/components';
    }
     ...
     *
     * Output example:
     * $contena-features: ("FEATURE_NEXT_1234": false, "FEATURE_NEXT_1235": true);
     *
    ...
    private function getFeatureConfigScssMap(): string
    {
        ...
        return \sprintf('$contena-features: (%s);', $featuresScss);
    }
    ...
    private function dumpVariables(array $config, string $themeId, string $channelId, Context $context): string
    {
        ...
        foreach ($this->packages as $key => $package) {
            $variables[\sprintf('contena-asset-%s-url', $key)] = \sprintf('\'%s\'', $package->getUrl(''));
        }
