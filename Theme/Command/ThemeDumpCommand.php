...
class ThemeDumpCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$themeEntity instanceof ThemeEntity) {
            $this->io->error('No theme found which is connected to a web channel');
    ...
    private function getThemeAssignmentInfos(): string
    {
        ...
        foreach ($themes as $theme) {
            ...
            $channels = $theme->getChannels()?->filterByTypeId(Defaults::CHANNEL_TYPE_WEB);
            ...
            if ($channelCount > 0) {
                $choices .=
                    \sprintf(
                        '%s || Assigned to: %s',
                        $themeName,
                        $channels ? implode(', ', $channels->map(static fn (ChannelEntity $channel) => $channel->getName())) : ''
                    );
                $choices .= \PHP_EOL;
                continue;
            }

            $choices .= \sprintf('%s || Not assigned to any web channel', $themeName);
            $choices .= \PHP_EOL;
        }
