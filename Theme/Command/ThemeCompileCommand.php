...
class ThemeCompileCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('only', 'o', InputOption::VALUE_REQUIRED | InputOption::VALUE_IS_ARRAY, 'Compile themes only for given channel ids')
            ->addOption('skip', 's', InputOption::VALUE_REQUIRED | InputOption::VALUE_IS_ARRAY, 'Skip compiling themes for given channel ids')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($this->themeProvider->load($context, $input->getOption('active-only')) as $channelId => $themeId) {
            ...
            $this->io->block(\sprintf('Compiling theme for channel: %s', $channelId));
